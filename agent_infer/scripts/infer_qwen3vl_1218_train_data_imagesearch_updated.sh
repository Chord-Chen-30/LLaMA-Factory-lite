...
MODEL_PATHS=(
    "./saves/qwen3_vl_30b_a8b_1216_imagesearch_updated/checkpoint-352"
)
...
for dataset in "${datasets[@]}"; do
    python src/infer_w_tools_mp_test.py --model_name_or_path ${MODEL_PATH} --data ${dataset} --date 2025-12-18 --mp 12 --overwrite 0 --log_level WARNING

    model_path=$(basename "$(dirname "$MODEL_PATH")")/$(basename "$MODEL_PATH")
    data_bn=$(basename "$dataset")
    data_bn=${data_bn%.jsonl}
    python src/qwen_max_score.py --data_path ./result/2025-12-18/${model_path}/${data_bn}_w_tools.jsonl --overwrite 1 &
done
