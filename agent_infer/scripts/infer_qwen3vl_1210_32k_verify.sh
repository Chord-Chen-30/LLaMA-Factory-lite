...
# 文件存在后再执行
FILE_PATH="./saves/qwen3_vl_30b_a8b_1210_32k_verify/all_results.json"
...
MODEL_PATHS=(
    "./saves/qwen3_vl_30b_a8b_1210_32k_verify/checkpoint-348" # 4 epochs
)
...
for dataset in "${datasets[@]}"; do
    python src/infer_w_tools_mp_test.py --model_name_or_path ${MODEL_PATH} --data ${dataset} --date 2025-12-10 --mp 12 --overwrite 1

    model_bn=$(basename "$MODEL_PATH")
    data_bn=$(basename "$dataset")
    data_bn=${data_bn%.jsonl}
    python src/qwen_max_score.py --data_path ./result/2025-12-10/${model_bn}/${data_bn}_w_tools.jsonl --overwrite 1 &
done
